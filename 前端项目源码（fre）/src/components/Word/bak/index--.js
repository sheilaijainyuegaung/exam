...
export function extractScores(htmlContent, title) {
  ...
  // 分值提取正则（全局匹配）
  let scorePattern = /[（(]\s*(\d+(?:\.\d+)?)\s*分\s*[）)]/g;
  let scorePattern2 = /[（(]\s*(?!\d+\s*[）)])(?:\d+(?:\.\d+)?\s*分|)\s*[）)]/g;
  ...
  function processText2(text, level = 0, prevCounter = 0) {	
    ...
    if (level >= maxLevel) {	
      ...
      const scores = Array.from(fixedText.matchAll(scorePattern2))
		...
		//console.log("processText scores:"+JSON.stringify(scores)); 
  ...
  function processText(text, level = 0, prevCounter = 0) {	
    ...
    if (level >= maxLevel) {
      ...
      // 查找所有分值
      const scores = Array.from(fixedText.matchAll(scorePattern))
        .map(match => parseFloat(match[1]));
		//console.log("processText scores:"+JSON.stringify(scores)); 
