...
//const top_margin = 6;//上边距
const top_margin_first = 6; // 第一页上边距
const top_margin_other = 6; // 其他页上边距
const left_margin = -20;//左边距
const right_margin = 2;//右边距
const bottom_margin = 6;//底边距
...
export const handlePdfDocument = async (file) => {
  try {
    ...
    // 获取基准页的尺寸信息（优先第二页，仅一页时使用第一页）
    const basePageNum = totalPages > 1 ? 2 : 1;
    const basePage = await pdf.getPage(basePageNum);
    const baseViewport = basePage.getViewport({ scale: 1.0 });
    
    // 计算基准页的毫米到像素转换比例
    const basePdfWidthMM = 210;   // A4宽度210mm
    const basePdfHeightMM = 297;  // A4高度297mm
    const basePxPerMMWidth = baseViewport.width / basePdfWidthMM;
    const basePxPerMMHeight = baseViewport.height / basePdfHeightMM;

    // 计算基准页的边距（像素）
    const baseTopMarginPx = Math.round(top_margin_other * basePxPerMMHeight);
    const baseLeftMarginPx = Math.round(left_margin * basePxPerMMWidth);
    const baseRightMarginPx = Math.round(right_margin * basePxPerMMWidth);
    const baseBottomMarginPx = Math.round(bottom_margin * basePxPerMMHeight);
	...
	//console.log("firstPageText:"+JSON.stringify(firstPageText)); 
    ...
    for (let i = 1; i <= totalPages; i++) {
	 ...
	 // 第一步：使用更高的分辨率渲染（提升清晰度基础）
	  ...
	  // 计算毫米到像素的转换比例
	  const pxPerMMWidth = canvas.width / pdfWidthMM;   // 宽度方向每毫米像素数
	  const pxPerMMHeight = canvas.height / pdfHeightMM; // 高度方向每毫米像素数

	  // 根据页码选择上边距
      const currentTopMargin = i === 1 ? top_margin_first : top_margin_other;
      const topMarginPx = Math.round(currentTopMargin * pxPerMMHeight);

	        
      // 使用基准页计算的边距（像素）转换成本页比例
      const leftMarginPx = Math.round(baseLeftMarginPx * (pxPerMMWidth / basePxPerMMWidth));
      const rightMarginPx = Math.round(baseRightMarginPx * (pxPerMMWidth / basePxPerMMWidth));
      const bottomMarginPx = Math.round(baseBottomMarginPx * (pxPerMMHeight / basePxPerMMHeight));

		

		/*

	  //console.log(`计算参数: 宽度=${canvas.width}px, 1mm=${pxPerMMWidth}px`);     
		// 定义留白大小
		const topMarginPx = Math.round(top_margin * pxPerMMHeight);    // 上留白6mm
		const leftMarginPx  = Math.round(left_margin * pxPerMMWidth);    // 左右留白2mm
		const rightMarginPx = Math.round(right_margin * pxPerMMHeight); // 下留白2mm
		const bottomMarginPx = Math.round(bottom_margin * pxPerMMHeight); // 下留白2mm
		*/

		//console.log(`边距计算: 上=${topMarginPx}px, 左=${leftMarginPx}px, 下=${bottomMarginPx}px`);


		// 计算裁切区域（确保不超出画布边界）
		let cropX = leftMarginPx;
		let cropY = topMarginPx;
		let cropWidth = Math.max(1, canvas.width - leftMarginPx - rightMarginPx);
		let cropHeight = Math.max(1, canvas.height - topMarginPx - bottomMarginPx);

		/*
		let cropWidth = Math.min(
			canvas.width - leftMarginPx - rightMarginPx, // 宽度：左边距 + 右边距
			canvas.width - cropX // 确保不超出右边界
		);
		let cropHeight = Math.min(
			canvas.height - topMarginPx - bottomMarginPx, // 高度：上边距 + 下边距
			canvas.height - cropY // 确保不超出下边界
		);
		*/


		// 创建新Canvas用于裁切后的图像（分辨率 1404 × 1872）
		const outputCanvas = document.createElement('canvas');
		outputCanvas.width = 1404;  // 目标宽度
		outputCanvas.height = 1872; // 目标高度
		...
		// 计算缩放比例（保持宽高比）
		const targetAspectRatio = outputCanvas.width / outputCanvas.height;
		const sourceAspectRatio = cropWidth / cropHeight;
		...
		if (sourceAspectRatio > targetAspectRatio) {
		  ...
		  drawHeight = cropHeight * (outputCanvas.width / cropWidth);
		  offsetX = 0;
		  offsetY = (outputCanvas.height - drawHeight) / 2; // 垂直居中
		} else {
		  // 源图像更高，以高度为基准缩放
		  drawHeight = outputCanvas.height;
		  drawWidth = cropWidth * (outputCanvas.height / cropHeight);
		  offsetX = (outputCanvas.width - drawWidth) / 2; // 水平居中
		  offsetY = 0;
		}
		...
		outputCtx.drawImage(
		  canvas,
		  cropX, cropY, cropWidth, cropHeight, // 源裁切区域
	...
	*/
	...
	console.log("mergedScore2:" + JSON.stringify(mergedScore)); 
	//return;
