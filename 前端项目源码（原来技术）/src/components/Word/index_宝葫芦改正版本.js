...
const left_margin = -2;//左边距
const right_margin = 2;//右边距
const bottom_margin = 2;//底边距
...
//const textTree = [[{"text":"看拼音写词语(16分)</p><p>(1)hétān()</p><p>(5)chànà()</p><p>(2)yíduàn()</p><p>(6)xiāngbiān()</p><p>(3)fànwéi()</p><p>(7)bùjǐn()</p><p>(4)kuòdà()</p><p>(8)zǐsè()</p>","score":16,"childText":[{"text":"(1分)","score":1,"childText":[]},{"text":"(1分)","score":1,"childText":[]},{"text":"(1分)","score":1,"childText":[]},{"text":"(1分)","score":1,"childText":[]},{"text":"(1分)","score":1,"childText":[]}]},{"text":"形近字组词(8分)</p><p>(1)镶()</p><p>(5)壤()</p><p>(2)努()</p><p>(6)怒()</p><p>(3)烂()</p><p>(7)拦()</p><p>(4)替()</p><p>(8)剃()</p>","score":8,"childText":[]},{"text":"选择正确读音(6分)</p><p>(1)太阳好像负着重荷(héhè)似的。</p><p>(2)一刹那(chàshà)间，这个深红的圆东西，忽然发出了夺</p><p>目的亮光。</p><p>第1页</p>","score":6,"childText":[]}]];
  ...
  function processText(text, level = 0, prevCounter = 0) {	
    ...
    if (level === 0) { // 中文数字标题
	  ...
	  //console.log("0.tempText:"+tempText); 
		...
		if(level==2 && counter==1){
			const match2 = tempText.match(singleScorePattern2);
			const [first, rest] = splitOnce(tempText, match2[0]); //这个地方出问题的

			console.log("level==2 && counter==1");
			//console.log("2.match:"+JSON.stringify(match2));
			console.log(">0.while push first:"+first+". level:"+level); 
			tempText = rest;
			segments.push(first);
		}else{
			console.log("level！==2 && counter！==1");
			const [first, rest] = splitOnce(tempText, match[0]); //这个地方出问题的
			console.log(">0.while push first:"+first+". level:"+level); 
			tempText = rest;
			segments.push(first);
		}
			...
			if (level == 1 && counter==1) {
				console.log("rest加上match[0]:"+match[0]); 
				tempText = match[0] + rest;
			}else{
				tempText = rest;
			}
			*/
    ...
    for (let i = 0; i < segments.length; i++) {	  
	  ...
	  if ((segment.indexOf("共") !== -1) && (level==1)) {
		  break;
	  }
      if (segment) {
  ...
  function processText2(text, level = 0, prevCounter = 0) {	
	...
	/*
	  // 匹配所有空括号
		...
		// 提取所有匹配项
		const scoreMatches = Array.from(fixedText.matchAll(scorePattern2));
		const emptyMatches = Array.from(fixedText.matchAll(emptyBracketPattern));

		// 判断逻辑
		let scores;

		if (emptyMatches.length >= 1 && scoreMatches.length >= 1) {
		  // 同时存在空括号和带分数括号：只保留空括号
		  scores = emptyMatches.map(match => parseFloat(match[0]));

		} else if (scoreMatches.length >= 1) {
		  // 仅有带分数括号
		  scores = scoreMatches.map(match => match[1] ? parseFloat(match[1]) : null);
		} else {
		  // 都没有
		  scores = [];
		}
      ...
      while (true) {
		...
		*/
		...
		if(level==2 && counter==1){
			...
			console.log("level==2 && counter==1");
			//console.log("2.match:"+JSON.stringify(match2));
			console.log(">0.while push first:"+first+". levle:"+level); 
			tempText = rest;
			segments.push(first);
		}else{
			console.log("level！==2 && counter！==1");
    ...
    for (let i = 0; i < segments.length; i++) {
      const segment = segments[i].trim();
	  //console.log("i:"+i);
	  console.log("for segment level:"+level + ",segment:"+segment);
	  if ((segment.indexOf("共") !== -1) && (level==1)) {
		  console.log("segment break:"+segment);
		  break;
	  }
      if (segment) {
